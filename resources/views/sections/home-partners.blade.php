        ...
        <div class="reveal text-center">
            ...
            <div class="mt-8 flex flex-wrap justify-center gap-3">
                @foreach ($partnerItems as $partner)
                    <span class="font-latin rounded-xl border border-line bg-mist px-5 py-3 text-sm font-semibold text-ink">
                        {{ $partner->title }}
                    </span>
                @endforeach
            </div>
            <a href="{{ route('licenses') }}" class="mt-8 inline-flex text-sm font-semibold text-brand hover:text-brand-deep">
                {{ __('Licenses & Partners') }} {{ app()->getLocale() === 'fa' ? '←' : '→' }}
            </a>
        </div>
    </div>
</section>
